...
#define c0(i, n) for (int i = n - 1; i >= 0; i--)
#define c1(i, n) for (int i = n; i >= 1; i--)
...
void dfs(int s, vi &va, vvi &graph,vi &ind){
    vpii vp;
    each(child,graph[s]) vp.pb(mp(ind[child],child));
    sortall(vp);
    each(chil,vp){
        auto child = chil.ss;
        if(va[child] <= va[s]){
            va[child] = va[s] + 1;
            dfs(child,va,graph,ind);
        }
    }
}

vector<int> topological_sort(int n, vector<set<int>> &graph, vector<int> &ind) {
...
void solve() {
    ...
    vi order = topological_sort(n,graph,ind);
    
    if(order[0] == -1) cout<<0<<endl;
    else{
        f1(i,sz(order)-1){
            debug(graph[order[i-1]]);
            debug(order)
            if(graph[order[i-1]].count(order[i]) == 0){
                cout<<0<<endl;
                return;
            }
        }
