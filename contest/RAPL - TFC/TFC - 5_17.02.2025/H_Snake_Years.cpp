...
ll make_number(vl& v){
    ll s = 0;
    fo(i,sz(v)) s = s*10 + v[i];
    return s;
}

void permu(int mask,vl& v, vl& cur){
    if(sz(cur) == sz(v)) {
        if(cur[0]) sub.insert(make_number(cur));
        else return;
    }else {
        fo(i,sz(v)){
            if(not isSet(mask,i)){
...
void solve() {
    ll n; cin>>n;
    vl v(n); cin>>v;
    fc(i,2001,1000000,12) uni.insert(i);
    rc(i,2001,0,12) uni.insert(i);    
