...
ll dpf(int i,int b,vl& v){
    
    if(i==sz(v)) return 0;
    if(dp[i][b]!=-1) return dp[i][b];

    ll mx = -INF;
    if(b==0) mx = max(mx, dpf(i+1,0,v) + abs(v[i]));
    if(b==0 or b==1) mx = max(mx, dpf(i+1,1,v) + 2 * v[i]);
    mx = max(mx, dpf(i+1,2,v) + abs(v[i]));
...
void solve() {
    ...
    mem(dp,-1);
    cout<<max({dpf(0,0,v),dpf(0,1,v),dpf(0,2,v)})<<endl;
}
